...
# Extract counts per sample from the active merged object
qc_cell_counts <- as.data.frame(table(merged_seurat$orig.ident)) %>%
  rename(Sample = Var1, Cells_After_QC = Freq) %>%
  mutate(
    # Pull initial counts saved prior to subsetting or reconstruct from metadata
    Cells_Before_QC = case_when(
      Sample == "GSM5599220_Norm1"  ~ 5831,
      Sample == "GSM5599225_Cancer1" ~ 6977,
      Sample == "GSM5599226_Cancer2" ~ 3473,
      TRUE ~ Cells_After_QC
    ),
